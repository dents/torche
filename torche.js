...
if (process.argv.length < 3) {
    console.error(`Usage: node ${__filename} <dir to verify>`);
    process.exit(1);
}
...
const firstTorrent = filesInDir.find((fName, ix, arr) => { return fName.endsWith('.torrent') });
...
walker.on("errors", function (root, nodeStatsArray, next) {
    console.error(`ERROR: ${nodeStatsArray}`);
    next();
});
walker.on("end", function () {
    ...
    for (let pieceIX = 0; pieceIX < t.pieces.length; ++pieceIX) {
        ...
        let totalHashed = 0;
        // const pieceOffset = pieceIX * t.pieceLength;
        let curPiecePos = 0;
        const filesInPiece = [];
        if (openedFile !== null) {
            filesInPiece.push({
        ...
        while (curPiecePos < t.pieceLength) {
            ...
            try {
                ...
                console.log(` CAUGHT ERROR READING: ${err}`);
            }
        }
